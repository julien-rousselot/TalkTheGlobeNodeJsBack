...
exports.downloadMaterial = exports.deleteMaterial = exports.updateMaterial = exports.sendIdForPayment = exports.getMaterialById = exports.createMaterial = exports.getPaidMaterials = exports.getFreeMaterials = exports.getAllMaterials = void 0;
const database_1 = require("../config/database");
const stripe_1 = __importDefault(require("stripe"));
const path_1 = __importDefault(require("path"));
const fs_1 = __importDefault(require("fs"));
const PDF_DIR = path_1.default.join(__dirname, "../uploads");
const stripe = new stripe_1.default(process.env.STRIPE_SECRET_KEY, { apiVersion: '2025-07-30.basil' });
...
const getFreeMaterials = async (_req, res) => {
    try {
        const result = await database_1.database.query(`
      SELECT 
        m.id AS material_id,
...
exports.getFreeMaterials = getFreeMaterials;
const getPaidMaterials = async (_req, res) => {
    try {
        const result = await database_1.database.query(`
      SELECT 
        m.id AS material_id,
...
exports.getPaidMaterials = getPaidMaterials;
const createMaterial = async (req, res) => {
    ...
    try {
        ...
        const materialResult = await client.query(`INSERT INTO materials (title, description, price, is_draft, publish_at, cover, pdf) 
       VALUES ($1, $2, $3, $4, $5::timestamp, $6, $7) RETURNING id`, [title, description, priceNum || null, isDraft === 'true', publish_at || null, coverUrl, pdfUrl]);
        ...
        if (files?.pictures?.length) {
            ...
            await client.query(`INSERT INTO pictures (material_id, url) VALUES ${placeholders}`, values);
        }
        ...
        if (selectedResource === 'paid') {
            ...
            await client.query(`UPDATE materials SET stripe_product_id = $1, stripe_price_id = $2 WHERE id = $3`, [stripeProductId, stripePriceId, materialId]);
        }
...
const getMaterialById = async (req, res) => {
    ...
    try {
        ...
        const materialResult = await database_1.database.query('SELECT * FROM materials WHERE id = $1', [id]);
        if (materialResult.rows.length === 0) {
            return res.status(404).json({ error: 'Matériel non trouvé' });
        }
        const material = materialResult.rows[0];
        // Récupérer les images liées
        const picturesResult = await database_1.database.query('SELECT id, url FROM pictures WHERE material_id = $1', [id]);
...
const sendIdForPayment = async (id) => {
    if (isNaN(id))
...
async function checkIfUserPaid(userId, materialId) {
    ...
    const result = await database_1.database.query("SELECT 1 FROM purchases WHERE user_id = $1 AND material_id = $2", [userId, materialId]);
